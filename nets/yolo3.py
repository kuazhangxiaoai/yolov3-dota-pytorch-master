...
class YoloBody(nn.Module):
    def __init__(self, config):
        ...
        final_out_filter3 = len(config["yolo"]["anchors"][2]) * (5 + config["yolo"]["classes"])
        self.last_layer3_conv = conv2d(128, 64, 1)
        self.last_layer3_upsample = nn.Upsample(scale_factor=2, mode='nearest')
        self.last_layer3 = make_last_layers([128, 256], out_filters[-4] + 64, final_out_filter3)
    ...
    def forward(self, x):
        ...
        out2, out2_branch = _branch(self.last_layer2, x2_in)

        x3_in = self.last_layer3_conv(out2_branch)
        x3_in = self.last_layer3_upsample(x3_in)
        x3_in = torch.cat([x3_in, x3])
        out3,_ = _branch(self.last_layer3,x3_in)
